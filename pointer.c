...
int intSize() {
  ...
  intPtr1 = intArray;
  /*add one to make Ptr2 single byte longer*/
  intPtr2 = intArray + 1;

  int firstAddress =(long) intPtr1;
  int secondAddress =(long) intPtr2;
...
int doubleSize() {
  ...
  int firstAddress =(long) doubPtr1;
  int secondAddress =(long) doubPtr2;
...
int pointerSize() {
  ...
  int firstAddress =(long) ptrPtr1;
  int secondAddress =(long) ptrPtr2;
...
int changeValue() {
  ...
  int * intPtr2;
  /*navigate to proper index to modify*/
  intPtr2 = intPtr1 + 5;
  *intPtr2 = (250 + 199);
...
int withinSameBlock(int *ptr1, int *ptr2) {
    /*the pointers will be in same block if the addresses are the same,
    except for the last 6 bits. The mask is used for the last 6 bits to compare equality*/
  int ptrMask = (1 << 31) >> 25;
  int firstAddress =(long) ptr1;
  int secondAddress =(long) ptr2;

  return (firstAddress & ptrMask) == (secondAddress & ptrMask);
}
...
int withinArray(int * intArray, int size, int * ptr) {
  /*if the pointer points to element in array, the sign of first address - second address
  will be the same as the sign of the difference between the last element in the array and the address 
  that ptr points to*/
  int firstAddress =(long) intArray;
  int secondAddress =(long) ptr;
  int ptrMask = (firstAddress - secondAddress) & ~(1 << 31);

  return size == (secondAddress & ptrMask);
}
...
int stringLength(char * s) {
  int count = 0;
  //increment through string until null terminator and increment count each step
  while(*s != '\0'){
	count++;
	s++;
  }
...
int endianExperiment(int* ptr) {
  ...
  /*convert 449449 to binary and write out the hexadecimal form, then start with lowest addresss
  and incrememnt by 1*/
  *bytePtr = 0xA9;
  bytePtr++;
